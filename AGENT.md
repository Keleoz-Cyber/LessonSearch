...
> 最后更新：2026-06-23 · 当前版本：v0.7.0+34
> 
...
| 表 | 说明 |
|----|------|
| users | 用户（role, real_name, password_hash） |
...
- **Task 状态同步修复（v0.6.0+）**：createTask 同步时携带完整状态字段（status/phase/current_*），服务端幂等更新已存在任务，修复断网/重启后 task 状态未同步导致提交失败的问题
