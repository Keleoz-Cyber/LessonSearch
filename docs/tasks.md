...
### P21：Bug修复 ✅ (v0.3.7)
- 竞态条件：修复记名快速点击导致状态丢失
- 布局问题：修复班级选择页需要滚动才能点按钮
- 乐观更新：状态先更新UI后异步保存，失败回滚

### P22：功能增强与UI优化 ✅ (v0.3.8)
- 总群汇报：按班级分组显示缺勤/迟到/请假/其他
- 跳转功能：复制文本后自动跳转微信/QQ
- 班级选择：单列布局，名称完整显示，无省略号
- 调试工具：统计卡片、操作日志持久化、重试失败
- 性能优化：记名页面RepaintBoundary隔离重绘

### P23：稳定性修复与优化 ✅ (v0.3.9)
- 数据库迁移：添加try-catch防止崩溃
- 同步机制：取消10条批量限制，支持全部同步
- 包可见性：修复微信/QQ跳转检测（Android 11+）
- 文本生成：总群汇报显示学号，全勤显示优化
- 班级选择：单列布局，完整显示班级名称
- 自动同步：间隔从30秒改为10秒
- 原子保存：auth_service使用Future.wait
- 同步去重：相同entity不重复入队
