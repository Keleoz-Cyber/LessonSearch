...
### 11.5 周汇总统计可能不准确 ✅ 已修复（v0.6.1）

**原问题**：周汇总统计的是 approved Submission 关联的实时 attendance_records，如果审核通过后记录被修改，统计结果会变化。

**修复措施**：
- 新增 `submission_snapshots` 表，管理员审核通过时自动锁定提交内容
- `get_week_summary()` 和 `export_week_excel()` 优先读取快照
- submission 级别 fallback：有快照用快照，没有则实时查询
- 旧数据兼容：历史 approved submission 无快照时自动 fallback
...
> 本文档完。所有内容基于 `v0.6.2` 代码实际实现整理，未做任何假设或推测。
