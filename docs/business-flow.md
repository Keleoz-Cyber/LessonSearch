...
- **网络错误**：标记为 failed，下次重试
- **404 错误**：标记为 synced（跳过）
- **最大重试**：5 次
- **失败后**：启动时自动重置 failed 记录并重试
...
根据代码实际实现，以下是需要确认或关注的风险：

### 11.1 数据一致性风险

**问题**：审核通过后，用户仍可在客户端修改记录，修改会同步到服务端，导致导出的 Excel 与实际提交时的数据不一致。

**代码证据**：
- `record_detail_page.dart::_updateStatus()` 没有检查 Submission 状态
- 服务端 `updateRecordByTaskStudent` 没有检查是否已提交

### 11.2 重复提交风险

**问题**：虽然服务端有 `uk_record` 唯一约束防止同一 record 被多次提交，但如果用户在客户端删除任务后重新创建，可能产生重复数据。

**代码证据**：
- `submission.py::create_submission()` 检查 `existing_submission_ids`
- 但只检查 record_id，不检查 task_id
...
**问题**：总群汇报的警告对话框只是 UI 提示，没有任何状态变更。用户可能误以为点击"确认最终记录"后就锁定了数据。

**代码证据**：
- `text_gen_page.dart::_copyAndOpenApp()` 只复制文本和跳转微信
- 没有修改任何数据库状态
