...
> 版本：0.1.0 | 更新日期：2026-04-03 | 仓库：https://github.com/Keleoz-Cyber/LessonSearch
...
查课 App 是一个面向课堂查课场景的 Flutter 应用（Android 优先），用于学习部日常考勤工作。支持点名、记名、查课记录查看与编辑、考勤文本生成等功能。
...
| 记名 | 逐人标记考勤状态（到课/缺勤/请假/其他），支持多班级切换 |
...
| `attendance_tasks` | 考勤任务（id UUID, type, status, phase, syncStatus, ...） |
...
```dart
const int announcementVersion = 2;       // 改公告后 +1
...
2. 检查更新功能为占位，未实现
3. 点名流程没有确认页和文本生成（直接完成）
4. 文本模板目前是代码常量，未做数据库配置化
5. 同步失败的记录没有 UI 提示详情
