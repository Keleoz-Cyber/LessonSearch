...
class SyncService {
  ...
  Future<void> processQueue() async {
    ...
    try {
      // getPendingSyncItems 返回 pending + failed(retryCount < max) 的记录
      ...
      for (final item in items) {
        try {
          await _processItem(item.entityType, item.entityId, item.action, item.payload);
          await _local.markSynced(item.id);
          successCount++;
          debugPrint('[Sync] OK: ${item.entityType}/${item.action} #${item.entityId}');
        } catch (e) {
  ...
  Future<void> _syncTask(String taskId, String action, Map<String, dynamic> payload) async {
    switch (action) {
      case 'create':
      ...
      case 'update':
        await _remote.updateTask(
          taskId,
          status: payload['status'] != null ? TaskStatus.fromString(payload['status']) : null,
          phase: payload['phase'] != null ? TaskPhase.fromString(payload['phase']) : null,
  ...
  Future<void> _syncRecord(String recordId, String action, Map<String, dynamic> payload) async {
    switch (action) {
      case 'create':
